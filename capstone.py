...
def base_mode():
	...
	def add_ranking():
		global name
		global normal_ranks
		global adventure_ranks
		# ranks = pickle.load(open("ranks.dat", "rb"))
		name = text_input_r.get()
		if game_status == "normal":
			normal_ranks = pickle.load(open("normalranks.dat", "rb"))
			normal_ranks[head.scores] = name
			text_input_r.delete(0, "end")
			lb_ranks_normal.delete(0,"end")
			scorelist = list(normal_ranks.keys())
			scorelist.sort()
			scorelist.reverse()
			for score in scorelist:
				rank = normal_ranks[score] + " SCORE: " + str(score)
				lb_ranks_normal.insert("end", rank)
			pickle.dump(normal_ranks, open("normalranks.dat", "wb"))

		elif game_status == "adventure":
			adventure_ranks = pickle.load(open("adventureranks.dat", "rb"))
			adventure_ranks[head.scores] = name
			text_input_r.delete(0, "end")
			lb_ranks_adventure.delete(0,"end")
			scorelist = list(adventure_ranks.keys())
			scorelist.sort()
			scorelist.reverse()
			for score in scorelist:
				rank = adventure_ranks[score] + " SCORE: " + str(score)
				lb_ranks_adventure.insert("end", rank)
			pickle.dump(adventure_ranks, open("adventureranks.dat", "wb"))
...
# have to fix the object
			for food in foods:
				if self.distance(food) < 20:
					os.system("afplay Apple_bite.mp3&")
					pen.clear()
					food.jump()
					body = Body()
					self.scores += 1
...
# the image source: http://rembound.com/articles/creating-a-snake-game-tutorial-with-html5